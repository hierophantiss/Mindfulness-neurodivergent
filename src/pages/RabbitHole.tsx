...
import { rabbitholeContent, type RabbitHoleArticle } from '../data/rabbitholeContent';
...
export default function RabbitHole() {
  ...
  // Η λίστα παράγεται από το rabbitholeContent — μοναδική πηγή.
  // Νέο κείμενο: προσθήκη εγγραφής εκεί, τίποτα εδώ.
  const articles = (Object.entries(rabbitholeContent) as [string, RabbitHoleArticle][])
    .sort(([, a], [, b]) => a.date.localeCompare(b.date))
    .map(([id, a]) => ({
      id,
      date: a.date,
      title: language === 'en' ? a.title.en : a.title.el,
      author: language === 'en' ? a.author.en : a.author.el,
      pages: language === 'en' ? a.en : a.el
    }));
 ...
 className="w-full md:col-span-2 glass-card rounded-[2rem] p-6 md:p-8 block text-left transition-all duration-300 active:scale-[0.98] hover:bg-white/[0.04] hover:border-teal-500/20 relative group"
          ...
          {articles.map((article) => {
            const isNew = article.date >= '2025-07';
            return (
              <button 
                key={article.id}
