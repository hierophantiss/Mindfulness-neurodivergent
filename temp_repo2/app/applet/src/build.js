                  ...
                  {videos.find(v => v.id === activeVideo) && (
                    <div className="flex flex-col gap-6">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {videos.find(v => v.id === activeVideo)?.description.points.map((point, idx) => (
...
repl = repl.replace(
  /export default function Sanctuary\(\) \{([\s\S]*?)const videos = \[/,
...
console.log('Successfully rewrote Sanctuary.tsx');
